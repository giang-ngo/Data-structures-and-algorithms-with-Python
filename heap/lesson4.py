class Heap:
    ...
    def __init__(self, capacity=10):
        ...
        self.__size: int = 0
        self.__data: list = [-999999] * self.__capacity if capacity <= 0 else [None] * self.__capacity
    ...
    def insert(self, value):
        ...
        if self.__size < self.__capacity:
            ...
            self.__data.append(value)  # Thêm phần tử vào cuối danh sách
            self.__capacity += 1  # Tăng dung lượng của heap
            self.sift_up(self.__size)  # Vun đống(sàng) lên
            self.__size += 1  # Tăng kích thước
            return True
    ...
    def sift_up(self, index):
        ...
        while parent_index >= 0 and self.__data[index] > self.__data[parent_index]:
    ...
    def update(self, old_node, new_node):
        """ Phương thức cập nhật một node trong heap. """
        index = self.find_node_index(old_node)  # tìm vị trí node cũ
        if index >= 0:  # nếu tìm thấy node cũ
            self.__data[index] = new_node  # cập nhật thành node mới
            self.sift_up(index)  # sàng lên từ vị trí của node mới
...
def test():
    ...
    heap.update(70, 100)
    print('==> Các phần tử trong heap sau khi update 70->100: ')
    heap.show_elements()
