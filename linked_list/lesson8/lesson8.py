from linked_list.lesson8.student import Student
...
class LinkedList:
    ...
    # phương thức tìm kiếm node có giá trị x trong danh sách
    def search(self, x: Student) -> Student | None:
        if self.is_empty():  # nếu danh sách rỗng
            return None  # rõ ràng không có kết quả cần tìm
        else:  # nếu danh sách không rỗng
            p = self.__head  # xét từ node đầu tiên trong danh sách
            while p is not None:  # chừng nào p khác None
                if p.data == x:  # nếu data của node p là x
                    return p.data  # trả về data của node p
                p = p.next  # cập nhật p
            return None  # cuối cùng không tìm thấy? trả về None

    # phương thức trộn hai danh sách không có thứ tự
    def merge_no_order(self, other):
        self.__tail.next = other.__head
        self.sort()

    # phương thức trộn hai danh sách đã sắp xếp thành một danh sách sắp xếp
    def merge_ordered(self, other):
        result = LinkedList()
        p = self.__head
        q = other.__head
        # khi cả hai danh sách chưa rỗng
        while p is not None and q is not None:
            if p.data <= q.data:
                result.insert_tail(p.data)
                p = p.next
            else:
                result.insert_tail(q.data)
                q = q.next
        # khi danh sách s1 còn chưa rỗng
        while p is not None:
            result.insert_tail(p.data)  # lấy các phần tử còn lại
            p = p.next
        # khi danh sách s2 còn chưa rỗng
        while q is not None:
            result.insert_tail(q.data)  # lấy các phần tử còn lại
            q = q.next
        return result
...
def test():
    ...
    linked_list_student1 = LinkedList()
    for student in students:
        linked_list_student1.insert_tail(student)

    linked_list_student2 = LinkedList()
    for student in students2:
        linked_list_student2.insert_tail(student)

    # in ra danh sách trước khi cập nhật
    print('==> Các phần tử trong DSLK1: ')
    linked_list_student1.show_elements()
    print('==> Các phần tử trong DSLK2: ')
    linked_list_student2.show_elements()

    # tiến hành trộn hai danh sách
    result = linked_list_student1.merge_ordered(linked_list_student2)
    print('==> DSLK sau khi trộn: ')
